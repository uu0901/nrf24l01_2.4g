...
*/
int flag = 0;
void checkRock1AdX_add(int rock1_ad_x)
{
    int newFlag = flag;
    static int prevFlag;        // 用于保存上次的 flag 状态
    // 检查是否达到3600左右
    if (rock1_ad_x >= 3590 && rock1_ad_x <= 4500 && flag == 0) {
        newFlag = 1;
        printf("Flag set to 1 at ROCK1_AD_X value: %d\n", rock1_ad_x);
    }
    // 检查是否回到2000~2100范围
    else if (rock1_ad_x >= 2000 && rock1_ad_x <= 2100 && flag == 1) {
        newFlag = 0;
        printf("Flag reset to 0 at ROCK1_AD_X value: %d\n", rock1_ad_x);
    }

    // 如果 flag 发生了改变，进行相应的处理
    if (newFlag != prevFlag) {
        if (newFlag == 0) {
            // 当 flag 从1变为0时，输出一个标志位
            printf("Transition detected: Flag changed from 1 to 0.\n");
            // 这里可以添加其他需要执行的动作，例如设置一个硬件标志位等
        }
        // 更新 flag 和 prevFlag
        flag = newFlag;
        prevFlag = newFlag;
...
void checkRock1AdX_dec(int rock1_ad_x)
{
    int newFlag = flag;
    static int prevFlag;        // 用于保存上次的 flag 状态
    // 检查是否达到3600左右
    if (rock1_ad_x >= 0 && rock1_ad_x <= 1800 && flag == 0) {
        newFlag = 2;
        printf("Flag set to 1 at ROCK1_AD_X value: %d\n", rock1_ad_x);
    }
    // 检查是否回到2000~2100范围
