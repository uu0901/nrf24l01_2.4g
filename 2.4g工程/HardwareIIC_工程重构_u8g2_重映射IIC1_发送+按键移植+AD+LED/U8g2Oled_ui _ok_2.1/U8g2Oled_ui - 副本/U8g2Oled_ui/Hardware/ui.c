...
int8_t led_x = 0, led_x_trg = 0;    //设置界面x 和设置界面x目标
int8_t led_y = 11, led_y_trg = 11;   //设置界面y 和设置界面y目标

int8_t led_frame_len, led_frame_len_trg;            //框框的宽度 和框框的宽度目标
int8_t led_frame_y, led_frame_y_trg;                //框框的y 和框框的y目标

int8_t led_Indic_y = 0, led_Indic_y_trg = 0;                           //指示器的y 和指示器y目标
int8_t led_Indic_x = 116, led_Indic_x_trg = 116;                           //指示器的y 和指示器y目标

int8_t led_Indic_vertical_x = 120, led_Indic_vertical_x_trg = 120;     //指示器垂线的x和指示器垂线的x目标
int8_t led_Indic_vertical_y = 0, led_Indic_vertical_y_trg = 0;     //指示器垂线的x和指示器垂线的x目标

int8_t led_Indic_Horizental_x = 118, led_Indic_Horizental_x_trg = 118;     //指示器垂线的x和指示器垂线的x目标
int8_t led_Indic_Horizental_y = 4, led_Indic_Horizental_y_trg = 4;     //指示器垂线的x和指示器垂线的x目标
//设置列表
SETTING_LIST list[] = 
{
    {"Led", 3},
...
void led_ui_show(int8_t offset_y, int8_t offset_Indic_y, int8_t offset_Indic_vertical_x)   //设置UI显示
{
    int list_len = sizeof(list) / sizeof(LED_LIST);//list_len=6

    for (int i = 0 ; i < list_len; i++)//0到5 list[0].str~list[5].str
    {
        //drawstr的xy坐标是左下角，所以y轴要偏移，这里默认的y轴offset_y形参传入的是seeting_y也就是18，也就是这个字符串的高度是18，这里默认+18是因为第一个字符的i是0
        
        u8g2_DrawStr(&u8g2, led_x + 6, offset_y + i * 13 + 11, list_led[i].str); // 第一段输出位置
    }
    
    u8g2_SetDrawColor(&u8g2,2);//异或显示，让框框反色，注意这里是要画RBox，Fram是不行的
    u8g2_DrawRBox(&u8g2, led_x, offset_y + led_frame_y, led_frame_len, 15, 6);//初始化框框的y轴是offset_y是18，此时框框的y轴和第一个文本的y轴重复了
    u8g2_SetDrawColor(&u8g2,1);//异或显示，让框框反色，注意这里是要画RBox，Fram是不行的
   
    if(backtoseeting == 1)//回到设置标志位，目的是让框框回弹到list[0]
    {
        backtoseeting = 0;//清除该标志位
        ui_run(&led_frame_y, &led_frame_y_trg, 10, 10);       //框框y轴ui运动函数 
    }
    else
    {
        ui_run(&led_frame_y, &led_frame_y_trg, 5, 4);       //框框y轴ui运动函数 
    }
    ui_run(&led_frame_len, &led_frame_len_trg, 10, 5);  //框框宽度ui运动函数

    u8g2_DrawVLine(&u8g2, offset_Indic_vertical_x, led_Indic_vertical_y, 64);//垂线 左下角xy
    for(int i = 0; i < list_len; i++)//画list条水平线
    {
        //y 8~120
        //x 118                  y = 8                      len = （120-x)*2 = 4                 
        //box_x= x - 2=116       box_y = y+2 = 10          boxlen = (120 - box_x)* 2 = 8   box_ = h/2 = 4   
        u8g2_DrawHLine(&u8g2, led_Indic_Horizental_x, led_Indic_Horizental_y + i*8, 4);//左下角xy
    }
    u8g2_DrawRBox(&u8g2, led_Indic_x, offset_Indic_y + led_Indic_y , 8, 8, 4);//左上角xy   // offset_Indic_y = 4  Indic_y 
    ui_run(&led_Indic_y, &led_Indic_y_trg, 5, 4);                      //框框宽度ui运动函数   Indic_y_trg 没运动一次偏移一个指示器
    ui_run(&led_Indic_x, &led_Indic_x_trg, 5, 4);  
    ui_run(&led_Indic_Horizental_y, &led_Indic_Horizental_y_trg, 5, 4);                      //框框宽度ui运动函数   Indic_y_trg 没运动一次偏移一个指示器
    ui_run(&led_Indic_vertical_y, &led_Indic_vertical_y_trg, 5, 4);  
...
{
    int newFlag = flag;         //默认flag是0也就是松手
    static int prevFlag;         //默认prevFlag是0也就是松手， 用于保存上次的 flag 状态
    // 检查是否达到3600左右，向上拨动检测
    if (rock1_ad_x >= 3590 && rock1_ad_x <= 4500 && flag == 0) //上一次是松手，这一次拨动才有效
    {
        newFlag = 1;//向上拨动标志
        printf("Flag set to 1 at ROCK1_AD_X value: %d\n", rock1_ad_x);
    }
    // 检查是否回到2000~2100范围，松手居中检测
    else if (rock1_ad_x >= 2000 && rock1_ad_x <= 2100 && flag == 1)//上一次是向上拨动，这一次松手才有效
    {
        newFlag = 0;//松手标志
...
void checkRock1AdX_dec(int rock1_ad_x)//检测X轴减，摇杆向下拨动为减
{
    ...
    static int prevFlag;        //默认prevFlag是0也就是松手， 用于保存上次的 flag 状态
    // 检查是否达到1800左右，向下拨动检测
    if (rock1_ad_x >= 0 && rock1_ad_x <= 1800 && flag == 0) //上一次是松手，这一次拨动才有效
    {
        newFlag = 2;//向下拨动标志
        printf("Flag set to 1 at ROCK1_AD_X value: %d\n", rock1_ad_x);
    }
    // 检查是否回到2000~2100范围，松手居中检测
    else if (rock1_ad_x >= 2000 && rock1_ad_x <= 2100 && flag == 2)//上一次是向下拨动，这一次松手才有效
    {
        newFlag = 0;//松手标志
        printf("Flag reset to 0 at ROCK1_AD_X value: %d\n", rock1_ad_x);
    }
        ...
        flag = 0;
            ...
            if (keynum == ROCK2_KEY_DobleClick)                              //key0
            {
                ...
                if(ui_select == 0)                          //选中了LED
                {
                    ...
                    setting_x_trg = -88;                   //设置设置界面的目标，向左偏移  
...
void led_proc(void)                         //LED处理函数
{
    ...
    int list_len = sizeof(list) / sizeof(LED_LIST);     //计算出列表的菜单个数
    u8g2_SetFont(&u8g2,u8g2_font_t0_11_mf);                         //设置字体
    // static uint8_t ledflag = 2;
    // if(ledflag == 0)    
    // {
    //     u8g2_DrawStr(&u8g2, 40, 12, "   ");
    //     u8g2_DrawStr(&u8g2, 40, 12, "on");
    // }
    // else                
    // {
    //     u8g2_DrawStr(&u8g2, 40, 12, "   ");
    //     u8g2_DrawStr(&u8g2, 40, 12, "off");
    // }




    // u8g2_DrawStr(&u8g2, 0, 12, "LED1:");
    // u8g2_DrawStr(&u8g2, 0, 24, "LED2:");
    // u8g2_DrawStr(&u8g2, 0, 36, "LED3:");
    // u8g2_DrawStr(&u8g2, 0, 48, "LED4:");
    // u8g2_DrawStr(&u8g2, 0, 60, "LED5:");
    // u8g2_DrawStr(&u8g2, 70, 12, "LED6:");
    // u8g2_DrawStr(&u8g2, 70, 24, "LED7:");
    if (keynum == ROCK1_KEY_SingleClick)                                    //id=1 是key0
    {
        ui_select ++;                                //选项+1
        if (ui_select >= list_len)                  //list_len=6  意味着框框要偏移6-1次，因为框框默认框选第一个菜单，因此限幅在list_len-1
    ...
    else  if (keynum == ROCK1_KEY_DobleClick)                                             //id=0 是key_up
    {
        ...
        if (y_select <= 0)                          //限幅菜单偏移3次
        {
            ...
            led_y_trg = -y_select * 13;                   //设置设置列表的文本偏移  光标向下和向上移动时，y_select都为正数，为了让菜单列表向上偏移，因此这里加了负号

            led_frame_y_trg = ui_select * 13;                    //设置框框的y轴偏移
            led_Indic_y_trg = ui_select * 4;                    //设置框框的y轴偏移
            led_frame_len_trg = list_led[ui_select].len * 13;        //设置框框的宽度偏移ui_select 0~5 对应list数组元素
    switch (ui_state)
    {
    ...
    ui_run(&led_y, &led_y_trg, 5, 4);           //设置界面的ui动作函数
    led_ui_show(led_y,led_Indic_y, led_Indic_vertical_x);                         //显示设置UI
}
...
void advalue_proc(void)                         //LED处理函数
{
        ...
        if(ledflag == 0)    u8g2_DrawStr(&u8g2, 80, 12, "on");
        else                u8g2_DrawStr(&u8g2, 80, 24, "off");
